module AccountHelper
  ...
  def isfriend(id)
    val=current_user.confirmed_friends.ids.include?(id)
    val
  end
  def canishow(id)
    ids=current_user.confirmed_friends.ids
    # user seeing his account page
    checker1= (id==current_user.id)
    # public account
    checker2= User.find(id).public_state?
    # private
    checker3= (User.find(id).private_state? && current_user.confirmed_friends.ids.include?(id))
    # protected
