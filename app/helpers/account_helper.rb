module AccountHelper
  ...
  def canishow(id)
    ...
    val=id==current_user.id||User.find(id).status=="public"||(User.find(id).status=="private" && Friend.exists?(user_id: current_user.id, friend_id: id, status: "accepted")||Friend.exists?(user_id: id, friend_id: current_user.id, status: "accepted"))||(User.find(id).status=="protected" && (Friend.exists?(user_id: ids, friend_id: id, status: "accepted")||Friend.exists?(user_id: id, friend_id: ids, status: "accepted")) ||Friend.exists?(user_id: current_user.id, friend_id: id, status: "accepted")||Friend.exists?(user_id: id, friend_id: current_user.id, status: "accepted")  )
    val
  end
end
